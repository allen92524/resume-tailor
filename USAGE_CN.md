...
> **为什么有区别？** Docker Desktop（macOS/Windows/WSL2）提供 `host.docker.internal` 自动路由到主机。在 Linux 原生 Docker 上，Ollama 只监听 `127.0.0.1`，容器需要 `--network host` 才能访问。`make docker-ollama` 命令会自动处理这个问题。
...
### Docker 中 Ollama 连接被拒绝（Linux 原生 Docker）

在 Linux 原生 Docker 上（非 Docker Desktop），Ollama 默认只监听 `127.0.0.1`。Docker 容器无法通过桥接网络访问。

**解决方法：** 使用 Makefile 快捷命令，自动处理网络配置：

```bash
make docker-ollama MODEL=ollama:gemma3
```

或手动使用：`docker run --network host ...`（完整命令见上方 Docker 章节）。

macOS、Windows 或 WSL2 使用 Docker Desktop 时不需要此操作，`host.docker.internal` 原生可用。
